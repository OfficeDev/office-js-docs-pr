...
Any settings previously saved by an add-in are loaded when it is initialized, so during the lifetime of the session you can just use the [set](https://dev.office.com/reference/add-ins/shared/settings.set) and [get](https://dev.office.com/reference/add-ins/shared/settings.get) methods to work with the in-memory copy of the settings property bag. When you want to persist the settings so that they are available the next time the add-in is used, use the **saveAsync** method.
...
The [refreshAsync](https://dev.office.com/reference/add-ins/shared/settings.refreshasync) method is only useful in coauthoring scenarios (which are only supported in Word) when other instances of the same add-in might change the settings and those changes should be made available to all instances.
