---
ms.date: 04/07/2022
title: Create an add-in from your Script Lab code
ms.prod: non-product-specific
description: Create an add-in from your Script Lab code
ms.localizationpriority: high
---

# Create an add-in from your Script Lab code

If you created a snippet in Script Lab, you may want to turn it into a stand-alone add-in. You can copy the code from Script Lab into a a project generated by the Yeoman Generator for Office (also called "Yo Office"). Then you can continue developing the code as an add-in that you can eventually deploy to others.

The steps in this article will refer to [Visual Studio Code](https://code.visualstudio.com/), but you can use any code editor that you prefer.
...
The previous command creates a new project folder named **basic-sample**. It is configured to run in the host you specified, and uses TypeScript. Script Lab uses TypeScript by default, but most of the snippets are actually just JavaScript. You can build a Yo Office JavaScript project if you prefer, but just be sure any code you copy over is JavaScript.

## Open the snippet in Script Lab

Let's use an existing snippet in Script Lab to learn how to copy a snippet to a Yo Office generated project.

1. Open Office (Word, Excel, PowerPoint, or Outlook) and then open Script Lab.
1. Select **Script Lab** > **Code**. If you are working in Outlook, you need to open an email message to read to see Script Lab on the ribbon.
1. In the Script Lab task pane, choose **Samples**. Then select a basic sample based on which Office host you are working in.
    - For Excel, or Word, choose the **Basic API Call (TypeScript)** sample.
    - For Outlook, choose the **Use add-in settings** sample.
    - For PowerPoint, choose the **Basic API call (Ofice 2013)** sample.
...
:::image type="content" source="../images/script-lab-script-tabs.png" alt-text="Screen shot of tabs in Script Lab.":::
...
1. Open the **taskpane.html** file, and add the following script tag.
...
If you created a snippet that has additional library dependencies, be sure to add them to the Yo Office project. You can find a list of all library dependencies on the **Libraries** tab in Script Lab.
...
If your snippet uses custom functions, you need to use the Yo Office custom functions template. Follow these steps to turn you custom functions into a standalone add-in.
...
1. Open the **/src/functions/functions.ts** file. If you are using a JavaScript project, the filename is **functions.js**.
1. In Script Lab, select the **Script** tab.
...
Once all the steps are complete you can run and test out your standalone add-in. Run the following command to get started.
