...
1. Select and hold (or right-click) on App Services and select **Create new Web App**. A Linux container is used by default.
...
1. Select and hold (or right-click) on App Services and select **Create new Web App... Advanced**.
1. Type a globally unique name for your web app and press **Enter**. The name must be unique across all of Azure and use only alphanumeric characters ('A-Z', 'a-z', and '0-9') and hyphens ('-').
...
1. In the **App Service** explorer in Visual Studio code, expand the node for the new app, select and hold (or right-click) **Application Settings**, and select **Add New Setting**:
...
1. Select and hold (or right-click) your App Service app and select **Open in Portal**.
...
1. In the VS Code **Explorer** browse to the `dist` folder. Select and hold (or right-click) the `dist` folder and select **Deploy to Web App..**.
