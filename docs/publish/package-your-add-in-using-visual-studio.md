...
2. In the **Where is your website hosted?** dropdown list, select or enter the HTTPS URL of the website that will host the content files of your add-in, and then choose **Finish**. (You must specify a URL that begins with the HTTPS prefix to complete this wizard.)
     ...
     [!include[HTTPS guidance](../includes/https-guidance.md)] If you want to use an HTTP endpoint for your website, you can open the XML manifest file in a text editor after the package has been created and replace the HTTPS prefix of your website with an HTTP prefix. 
