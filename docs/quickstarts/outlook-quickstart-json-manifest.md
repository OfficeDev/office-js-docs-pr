...
- [.NET Configure your Office Add-in to use a shared runtime](https://dotnet.microsoft.com/download/dotnet/6.0/Configure your Office Add-in to use a shared runtime) for Windows. One of the tools used in the preview runs on .NET.
...
1. When an add-in command runs code instead of opening a task pane, it must run the code in a Configure your Office Add-in to use a shared runtime that is separate from the embedded webview in which task pane code runs. So the manifest must specify an additional Configure your Office Add-in to use a shared runtime. Scroll to the property `extension.Configure your Office Add-in to use a shared runtimes` and add the following object to the `Configure your Office Add-in to use a shared runtimes` array. Be sure to put a comma after the object that is already in the array. Note the following about this markup:
    ...
    ```json
    {
        "id": "ComposeCommandsConfigure your Office Add-in to use a shared runtime",
        "type": "general",
        "code": {
...
1. The **Show Taskpane** button appears when the user is reading an email, but the button for adding text should only appear when the user is composing a new email (or replying to one). So the manifest must specify a new ribbon object. Scroll to the property `extension.ribbons` and add the following object to the `ribbons` array. Be sure to put a comma after the object that is already in the array. Note the following about this markup:
    ...
    - The value of the `tabs[0].groups[0].controls[0].actionId` must be exactly the same as the value of `actions[0].id` property in the Configure your Office Add-in to use a shared runtime object you created in an earlier step.
