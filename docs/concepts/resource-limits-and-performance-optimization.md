...
- A range is limited to **5,000,000** cells for read operations. When a read exceeds this limit, the API will either return `null` or throw an error. For more information, see [Payload size limit best practices](../excel/performance.md#payload-size-limit-best-practices).
