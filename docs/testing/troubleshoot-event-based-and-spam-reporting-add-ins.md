...
On Windows clients (except for new Outlook on Windows), if your event-based add-in uses webpack to bundle files and your add-in continuously times out, we recommend configuring your add-in's webpack dev server to serve static files. This approach prevents webpack from adding code to your add-in's files that may be incompatible with the JavaScript runtime. Configure your add-in to use static files as follows:
