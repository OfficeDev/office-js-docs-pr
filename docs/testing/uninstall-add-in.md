...
- **Microsoft 365 Agents Toolkit CLI**: Use the `teamsapp uninstall` command at the same command line where you started the session with `teamsapp install`. For more information, see [Sideload with the Microsoft 365 Agents Toolkit CLI](../testing\sideload-add-in-with-unified-manifest.md#sideload-with-the-microsoft-365-agents-toolkit-cli-command-line-interface).
...
> [!TIP]
> There is a fast way to remove a ghost add-in on Windows computers if the add-in was installed with the Agents Toolkit CLI. Try this first, and if it works, you can skip the remainder of this section.
>
