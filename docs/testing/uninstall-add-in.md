...
- **Microsoft 365 Agents Toolkit CLI**: Use the `teamsapp uninstall` command at the same command line where you started the session with `teamsapp install`. For more information, see [Sideload with Microsoft 365 Agents Toolkit CLI](../testing\sideload-add-in-with-unified-manifest.md#sideload-with-microsoft-365-agents-toolkit-cli-command-line-interface).
