...
- **Function commands**: The button or menu item runs any arbitrary JavaScript. The code almost always calls APIs in the Office JavaScript Library, but it doesn't have to. This type of add-in typically displays no UI other than the button or menu item itself. Note the following about function commands:
   ...
   - The Configure your Office Add-in to use a shared runtime in which the function command runs is a full [browser-based Configure your Office Add-in to use a shared runtime](../testing/Configure your Office Add-in to use a shared runtimes.md#browser-Configure your Office Add-in to use a shared runtime). It can render HTML and call out to the Internet to send or get data.
