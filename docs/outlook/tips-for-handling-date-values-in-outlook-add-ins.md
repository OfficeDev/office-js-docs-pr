...
|API member|Time zone representation|Example in Outlook on the web, on mobile devices, and on new Windows client (preview)|Example in Outlook on classic Windows client and on Mac|
|--------------|----------------------------|-------------------------------------|-------------------|
|[Office.context.mailbox.userProfile.timeZone](/javascript/api/outlook/office.userprofile?view=outlook-js-preview&preserve-view=true#outlook-office-userprofile-timezone-member)|In classic Outlook on Windows and Outlook on Mac, this property returns the client computer time zone. In Outlook on the web, on mobile devices, and in new Outlook on Windows (preview), this property returns the EAC time zone.|PST|EST|
