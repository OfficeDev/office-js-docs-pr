...
> [!NOTE]
> If the add-in is using the [Teams manifest for Office Add-ins (preview)](json-manifest-overview.md), there is some manifest configuration, but Microsoft Graph scopes aren't specified. SSO-enabled add-ins that the Teams manifest can be sideloaded, but can't be deployed in any other way at this time.
