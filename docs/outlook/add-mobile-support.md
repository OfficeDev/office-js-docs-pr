...
Currently, add-ins in Outlook mobile only support activation when reading messages. Add-ins aren't activated when composing messages or when viewing or composing appointments. However, there are some exceptions. To learn more, see the "What's different on mobile" section of [Add-ins for Outlook on mobile devices](outlook-mobile-addins.md#whats-different-on-mobile).
