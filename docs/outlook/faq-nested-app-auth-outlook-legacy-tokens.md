...
You can determine if the signed-in user is an Exchange Online account, or on-premise Exchange account. Use the [Office.UserProfile.accountType](/javascript/api/outlook/office.userprofile?view=outlook-js-preview) property. If the account type property value is **enterprise**, then the mailbox is on an on-premises Exchange server. Note that volume-licensed perpetual Outlook 2016 doesn’t support the **accountType** property. To workaround this, call the [ResolveNames](/exchange/client-developer/web-service-reference/resolvenames-operation) operation in Exchange Web Service (EWS) in the Exchange on-premise server to get the recipient types.
