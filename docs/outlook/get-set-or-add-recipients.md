...
> [!NOTE]
> In Outlook on mobile devices, the `setAsync` method isn't supported in quick reply mode (the reply field at the bottom of a message).
...
> [!NOTE]
> In Outlook on mobile devices, the `addAsync` method isn't supported in quick reply mode (the reply field at the bottom of a message).
