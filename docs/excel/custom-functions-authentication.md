...
description: Learn how to authenticate users in Excel custom functions that run in a JavaScript-only runtime and share access tokens with a task pane.
ms.date: 07/10/2026
ms.topic: how-to
...
1. The sign-in page returns an access token to the dialog.
1. The dialog calls [Office.ui.messageParent](/javascript/api/office/office.ui#office-office-ui-messageparent-member(1)) to send the access token to the custom function. For more information, see [Send information from the dialog box to the host page](../develop/dialog-api-in-office-add-ins.md#send-information-from-the-dialog-box-to-the-host-page).
1. The custom function stores the access token in `OfficeRuntime.storage`.
1. The add-in's task pane gets the token from `OfficeRuntime.storage`.
...
> [!NOTE]
> The JavaScript-only runtime uses a dialog object that's slightly different from the dialog object in the browser runtime used by task panes. Both are referred to as the "Dialog API", but to authenticate users in the JavaScript-only runtime, use [OfficeRuntime.displayWebDialog](/javascript/api/office-runtime#office-runtime-officeruntime-displaywebdialog-function(1)), not [Office.ui.displayDialogAsync](/javascript/api/office/office.ui#office-office-ui-displaydialogasync-member(1)).

### Dialog box API example

In the following code sample, `getTokenViaDialog` uses `OfficeRuntime.displayWebDialog` to display a dialog box. This sample shows method capabilities and isn't a full authentication implementation.
...
If the custom function authenticates, it receives an access token that it must store in `OfficeRuntime.storage`. The following code sample shows how to call `storage.setItem` to store a value. The `storeValue` function is a custom function that stores a value from the user. You can modify it to store any token value you need.
