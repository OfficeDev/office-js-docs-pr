...
This article covers debugging only for custom functions with the **[add-in only manifest](../develop/add-in-manifests.md)** that **don't use a [shared runtime](../testing/runtimes.md#shared-runtime)**. For unified manifest and shared runtime scenarios, see [Overview of debugging Office Add-ins](../testing/debug-add-ins-overview.md).
