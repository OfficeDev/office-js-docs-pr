...
| `linkedEntityLoadService` | Boolean | No<br/><br/>Default value is `false`. | If `true`, the custom function registers a data domain that loads data for linked entities. A function can’t have both `excludeFromAutoComplete` and `linkedEntityLoadService` properties set to `true`. For more information, see [Linked entity load service function](excel-data-types-linked-entity-cell-values.md#linked-entity-load-service-function). |
