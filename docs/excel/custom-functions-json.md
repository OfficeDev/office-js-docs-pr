...
|  `volatile`  | boolean | No <br/><br/>Default value is `false`. | If `true`, the function will recalculate each time Excel recalculates, instead of only when the formula's dependent values have changed. Functions cannot be both streaming and volatile. If marked with both options as `true`, the function will default to streaming `true` and volatile `false`. |
