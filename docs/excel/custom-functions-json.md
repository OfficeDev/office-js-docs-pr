...
|  `requiresAddress`  | boolean | No <br/><br/>Default value is `false`. | <br /><br /> If `true`, the function will be passed the address of the cell that invoked your custom function. In this option, Excel will call the JavaScript function with the last parameter being an 'invocatationContext' paramter. In the body of the function, if `requiresAddress` is set, you can access `context.address` member. A function cannot be both streaming and require an address. For more information, see [Determine which cell invoked your custom function](https://docs.microsoft.com/en-us/office/dev/add-ins/excel/custom-functions-overview#determine-which-cell-invoked-your-custom-function). |
