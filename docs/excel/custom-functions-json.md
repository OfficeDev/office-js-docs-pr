...
| `requiresAddress` | boolean   | No <br/><br/>Default value is `false`. | If `true`, your custom function can access the address of the cell that invoked your custom function. To get the address of the cell that invoked your custom function, call the `address` property of the invocation object. For example, if your invocation object is named `invocation`, call `invocation.address` in your custom function. When using `requiresAddress`, the `invocation` parameter must be the last parameter passed to the function. A function can't use both the `stream` and `requiresAddress` properties. |
