...
When you group a range, Excel adds outline controls (the plus and minus buttons) outside the grouped range. The control appears on the row or column after the grouped range. For example, if you group rows 4-9, the control appears on row 10. When users click the minus button, rows 4-9 collapse; when they click the plus button, those rows expand again.
