---
title: 'Tutorial: Build a message compose Outlook add-in with unified manifest'
description: In this tutorial, you will build an Outlook add-in, with the unified manifest, that inserts GitHub gists into the body of a new message.
ms.date: 01/12/2024
ms.service: outlook
...
# Tutorial: Build a message compose Outlook add-in with the unified manifest for Microsoft 365
...
- Outlook 2016 or later on Windows (connected to a Microsoft 365 account)
...
Make the following updates in the **manifest.json** file to specify some basic information about the add-in.
...
Now that you've verified the base add-in works, you can customize it to add more functionality. 

By default, the manifest only defines buttons for the read message window. Let's update the manifest to remove the buttons from the read message window and define two new buttons for the compose message window:

- **Insert gist**: a button that opens a task pane

- **Insert default gist**: a button that invokes a function

### Change the context where the buttons appear

1. Open the **manifest.json** file.

1. In the "extensions.runtimes" array, there are two runtime objects. For the second one, with the "id" of "CommandsRuntime", change the "actions.id" to "insertDefaultGist". This is the name of a function that you create in a later step.

1. Change the item in the "extensions.ribbons.contexts" array to "mailCompose". This means the the buttons will appear in only a new message or reply window.

1. Change the "extensions.ribbons.tabs.groups" array has a group object in it. Make the following changes to this object:
