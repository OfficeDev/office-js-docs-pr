...
1. Select the **Browse** tab and search for **Microsoft.TypeScript.MSBuild**. Install this package to the ASP.NET web project, or update it if it's already installed. This will ensure the project will transpile to JavaScript when the build runs.
...
1. In **Solution Explorer** create a new file named **tsconfig.json** under the web project. The web project has the name you created the project with and the text **Web** appended to the name.
1. Enter the following JSON into the **tsconfig.json** file.
...
- [](../quickstarts/excel-quickstart-jquery.md)
