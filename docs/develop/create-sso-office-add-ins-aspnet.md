...
1. In the root of the **Begin** folder, open the solution (.sln) file in **Visual Studio**. Select and hold (or right-click) the top node in **Solution Explorer** (the Solution node, not either of the project nodes), and then select **Set startup projects**.
