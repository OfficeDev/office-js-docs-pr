...
> - The `messageParent` function is one of *only* two Office JS APIs that can be called in the dialog box. For information about it, see [Specify Office hosts and API requirements](specify-office-hosts-and-api-requirements.md).
> - The other JS API that can be called in the dialog box is `Office.context.requirements.isSetSupported`. However, in this situation, this API isn't supported in Outlook 2016 one-time purchase (that is, the MSI version).
