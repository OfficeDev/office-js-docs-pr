...
|WOPICatalog (third-party [WOPI](/microsoft-365/cloud-storage-partner-program/online/) hosts)| The AppSource asset ID of the add-in (see Note). | "wopicatalog". Use this value for add-ins that are published in App Source and are installed in third-party WOPI hosts. For more information, see [Enable Office Web Add-Ins provided by Host](/microsoft-365/cloud-storage-partner-program/online/overview). | The version in the add-in manifest.|
