...
- It is a completely separate browser instance from the task pane, meaning:
  ...
  - It does not share the same session storage (that is; the [Window.sessionStorage](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage) property) as the task pane.
...
![An image showing the relationship of the task pane and dialog box browser processes.](../images/taskpane-dialog-processes.gif)
...
The fact that the Office dialog box and the task pane run in different browser, and JavaScript runtime, instances means that you must use many authentication/authorization libraries in the way that is different from how they are used when authentication and authorization can take place in the same window. The following sections describe the main ways that you usually cannot use these libraries and the way that you *can* use them.
