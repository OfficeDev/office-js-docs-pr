...
1. Under **Manage**, select **App registrations** > **New registration**. On the **Register an application** pane, set the values as follows.

    * Set **Name** to `<add-in-name>`.
    * Set **Supported account types** to **Accounts in any organizational directory (any Azure AD directory - multitenant) and personal Microsoft accounts (e.g. Skype, Xbox)**.
    * Set the Redirect URI based on the specific redirect configured in your application. For example, for the [Create an ASP.NET Office Add-in that uses single sign-on](../develop/create-sso-office-add-ins-aspnet.md) article and sample, you would set it to `https://localhost:44355/AzureADAuth/Authorize`.
...
1. Select **Certificates & secrets**. Then on the **Client secrets** tab, select **New client secret**.
...
1. Select **Add**. The new secret is created the value is temporarily displayed.
...
1. Select **Set** to generate an application ID URI.
    ...
    The section for setting the application ID URI appears with a generated Application ID URI in the form `api://<application-id>`.

1. Enter the application ID URI in the format explained here.

    :::image type="content" source="../images/azure-portal-app-id-uri-details.png" alt-text="An example URI with localhost port set to 3000.":::

    * The **Application ID URI** is pre-filled with app ID (GUID) in the format `api://<AppID>`.
    * The application ID URI format should be: `api://<fully-qualified-domain-name>/<application-id>`
    * Insert the `fully-qualified-domain-name` between `api://` and `<AppID>` (which is a GUID). For example, `api://contoso.com/<AppID>`.
    * If you are using localhost, then the format should be `api://localhost:<port>/<AppID>`, for example, `api://localhost:6789/c6c1f32b-5e55-4997-881a-753cc1d563b7`.
...
1. Select the checkbox for `api://<fully-qualified-domain-name>/<application-id>/access_as_user` in **Authorized scopes**.
