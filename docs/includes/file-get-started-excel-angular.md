...
- Install [Browsersync](https://browsersync.io/docs) globally.
...
1. Use the Angular CLI to generate your Angular app. From the terminal, run the following command:

    ```bash
    ng new my-addin
    ```

2. From the terminal, run the following command to identify the folder where global **npm** libraries are installed:
    ...
    > [!TIP]
    > The first line of output that's generated by this command specifies the folder where global **npm** libraries are installed. For 

3. Navigate to the `{global libraries folder}/node_modules/browser-sync/lib/server/certs` folder. Then, copy the **server.key** file and the **server.crt** file from that folder and paste the copied files into the app folder that you created previously in step 1.
...
3. Open the manifest file (i.e., the file in the root directory of your app with a name ending in "manifest.xml"). Replace all occurrences of `https://localhost:3000` with `https://localhost:4200` and save the file.
