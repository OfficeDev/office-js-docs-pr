...
- If the add-in runs on a host and platform that supports custom contextual tabs, then the custom core tab won't appear on the ribbon. Instead, the custom contextual tab will be created when the add-in calls the `requestCreateControls` method.
- If the add-in runs on a host or platform that *doesn't* support `requestCreateControls`, then the custom core tab does appear on the ribbon.
