...
|9021|Save Error|Connection error occurred while trying to save the item on the server.|The item couldn't be saved. This could be due to a server connection error if using Online Mode in Outlook desktop, or due to an attempt to save a draft item that was deleted from the Exchange server.|
